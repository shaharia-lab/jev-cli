...
`exit_code`, `error_type`, `message`, `hint`, `request_id`, ...). With --merge, both kinds also \
carry `row` right after `id`: the whole input row as read (for CSV, an object of strings), so the \
records need no join by id; the output grows by about the size of the input. Filter records \
with `jq`, e.g. `jq -c 'select(.status == \"ok\" and .answers.is_urgent.noul >= 0.6)'`. A failed \
row does not stop the run unless --fail-fast or --max-errors says so. The summary goes to \
stderr (one `{\"summary\": ...}` JSON line when the output is for a program) and to --summary-json: \
`rows_total`, `ok`, `failed`, `skipped`, `already_ok` (not sent: --resume found them \
answered), `input_tokens`, `cost_usd` (an estimate), `wall_time_ms`, `rows_per_second`, \
`retries`, `models` and `stopped_by` \
